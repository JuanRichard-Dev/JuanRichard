...
LAYOUT_DEFAULTS: dict = dict(
    ...
    font=dict(family="Inter, sans-serif", color="#D9E6F2", size=12),
    margin=dict(l=44, r=24, t=70, b=50),
    legend=dict(
...
def _apply_layout(
    fig: go.Figure, title: str = "", height: int = 380, *, subtitle: str = "", is_heatmap: bool = False
) -> go.Figure:
        ...
        **LAYOUT_DEFAULTS,
        ...
        "xaxis": {
            **LAYOUT_DEFAULTS["xaxis"],
            ...
            "tickangle": -45 if is_heatmap or "mes" in str(title).lower() else 0,
        },
        "yaxis": {
...
def heatmap(
    ...
    return _apply_layout(fig, title, height, is_heatmap=True)
...
def waterfall_chart(
    ...
    fig = go.Figure(go.Waterfall(
        ...
        connector=dict(line=dict(color="rgba(59, 130, 246, 0.2)", width=1)),
        textposition="outside",
        cliponaxis=False,
        textfont=dict(size=11, color="#EAF2FC"),
        text=[
...
def bullet_chart(
    ...
    *,
    ...
    """Compact bullet chart with target and semantic performance ranges."""
    value = float(value)
    target = float(target)
    ...
    if lower_is_better:
        steps = [
            dict(range=[0, target], color="rgba(16,185,129,0.16)"),
            dict(range=[target, min(max_value, target * 1.35)], color="rgba(245,158,11,0.14)"),
            dict(range=[min(max_value, target * 1.35), max_value], color="rgba(220,38,38,0.13)"),
        ]
        bar_color = COLORS["green"] if value <= target else COLORS["orange"] if value <= target * 1.35 else COLORS["red"]
    else:
        warning_start = max(0.0, target * 0.75)
        steps = [
            dict(range=[0, warning_start], color="rgba(220,38,38,0.13)"),
            dict(range=[warning_start, target], color="rgba(245,158,11,0.14)"),
            dict(range=[target, max_value], color="rgba(16,185,129,0.16)"),
        ]
        bar_color = COLORS["green"] if value >= target else COLORS["orange"] if value >= warning_start else COLORS["red"]
    ...
    fig = go.Figure(go.Indicator(
        ...
        delta={
            ...
            "relative": False,
            "valueformat": ".1f",
            "increasing": {"color": COLORS["red"] if lower_is_better else COLORS["green"]},
            "decreasing": {"color": COLORS["green"] if lower_is_better else COLORS["red"]},
        },
        number={"suffix": value_suffix, "valueformat": ".1f", "font": {"size": 28, "color": "#F1F5F9"}},
        title={"text": title, "font": {"size": 14, "color": "#CBD5E1"}},
        gauge={
            "shape": "bullet",
            "axis": {"range": [0, max_value], "tickfont": {"size": 10, "color": "#64748B"}},
            "bar": {"color": bar_color, "thickness": 0.55},
            "steps": steps,
            "threshold": {"line": {"color": "#F8FAFC", "width": 3}, "thickness": 0.85, "value": target},
            "bgcolor": "rgba(0,0,0,0)",
            "borderwidth": 0,
        },
    ))
...
def _altair_dark_theme() -> dict:
    ...
    return {
        ...
        "axis": {
            ...
            "domainColor": "rgba(59, 130, 246, 0.2)",
        ...
        "range": {
            "category": ["#2563EB", "#10B981", "#0EA5E9", "#F97316", "#A855F7", "#EC4899"],
            "ordinal": ["#2563EB", "#10B981", "#0EA5E9", "#F97316", "#A855F7"],
        },
    }
    ...
    *,
    ...
    chart = (
        ...
        .encode(
            ...
            color=alt.Color(f"{color_col}:N", title=color_col.replace("_", " ").title()) if color_col else alt.value("#2563EB"),
            tooltip=[
